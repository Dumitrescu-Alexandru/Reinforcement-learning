...
class Agent(object):
    ...
    def update_estimator(self):
        ...
        # Task 2: TODO: Reformat data in the minibatcha
        states = [samples[i].state for i in range(len(samples))]
        action = [samples[i].action for i in range(len(samples))]
        next_states = [samples[i].next_state for i in range(len(samples))]
        rewards = [samples[i].reward for i in range(len(samples))]
        dones = np.array([samples[i].done for i in range(len(samples))])
        ...
        targets = next_qs * self.gamma + rewards
    ...
    def single_update(self, state, action, next_state, reward, done):
        ...
        max_ = self.q_functions[0].predict(featurized_next_state)
        for q_fcn in self.q_functions:
            next_qs = q_fcn.predict(featurized_next_state)
            if max_[0] < next_qs[0]:
                max_ = next_qs
        next_qs = np.zeros(1) if done else max_
        # Calculate the updated target Q- values
        # Task 1: TODO: Calculate target based on rewards and next_qs
        target = next_qs * self.gamma + reward
