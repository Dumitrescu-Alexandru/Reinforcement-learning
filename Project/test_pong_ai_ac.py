...
import wimblepong
import torch
from models.ac_models import Agent
import pickle
...
parser.add_argument("--ac_model", default=1, type=int, help="Number times to step with the same action")
...
for i in range(0, episodes):
    ...
    while not done:
        ...
        augmented_observation = augment(state_list, args.history)
        action, action_probabilities, value = player.get_action(augmented_observation)
        # observation_tensor = torch.from_numpy(observation).float().to(train_device)
        previous_observation = observation

        # Perform the action on the environment, get new state and reward
        observation, rew1, done, info = env.step(action.detach().cpu().numpy())
        observation = preprocess(observation)
        # state_list.append(observation)
        # Store action's outcome (so that the agent can improve its policy)
        # rew1 = 0.05 if not done else rew1
        player.store_outcome(previous_observation, action_probabilities, rew1, value)
        ttd += 1
        state_list.append(preprocess(observation))
